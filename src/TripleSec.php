...
class TripleSec
{
    ...
    protected static function hmac(string $data, string $key, callable $algoFn, int $algoBlockSizeInBytes): string
    {
        ...
        if (strlen($key) < $algoBlockSizeInBytes) {
            $key = $key . str_repeat(chr(0x00),
                    $algoBlockSizeInBytes - strlen($key)); // keys shorter than blocksize are zero-padded
        }
    ...
    {
        $hashFn = function ($data) {
            return hash('sha3-512', $data, true);
        };
    ...
    protected static function getStretchedKeys($key, $salt)
    {
        ...
        $keyMaterial = sodium_hex2bin(scrypt($key, $salt, static::SCRYPT_N, static::SCRYPT_R, static::SCRYPT_P,
            $totalSize));
